<template>
        ...
        <transition name="slide-fade" mode="out-in">
          <div class="popup-content help-popup" :key="step">
            <div v-if="step === 0" class="text-center">
              <h2 class="text-primary mb-3">📌 Asignación Óptima</h2>
              <p>
                ¿Deseas ver una guía rápida para utilizar el algoritmo de <strong>Asignación Óptima</strong> en tu grafo?
              </p>
            ...
            <div v-else>
              <h2 class="text-primary mb-3">📘 Paso {{ step }} - Asignación</h2>
              <img
                class="img-fluid rounded mb-3 border"
                :src="`/images/AssignmentStep${step}.png`"
                :alt="`Paso ${step}`"
              />
  ...
  export default {
    ...
    data() {
      return {
        ...
        visible: true,
        steps: [
          `✅ <strong>Paso 1:</strong> Crea un grafo bipartito, asegurandote que tener pesos`,
          `✅ <strong>Paso 2:</strong> presiona el boton "asignacion" del menu lateral y se mostraran los resultados del algoritmo`,
  ...
  <style scoped>
  /* Mismos estilos que HelpJohnsonPopup */
  .popup-overlay {
    position: fixed;
