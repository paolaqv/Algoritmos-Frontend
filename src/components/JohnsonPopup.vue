...
export default {
  ...
  methods: {
    // Función para quitar decimales (ej. 3.0 → 3)
    noDecimals(value) {
      if (value === undefined || value === null) return 0;
      return parseInt(value);
    },

    // Función que calcula la posición del texto usando el punto origen y la dirección hacia el destino.
    // distAlong: distancia a lo largo de la arista desde el origen.
    // distNormal: desplazamiento perpendicular (negativo: arriba; positivo: abajo).
    getTextPosition(x1, y1, x2, y2, distAlong = 10, distNormal = -10) {
      const dx = x2 - x1;
      const dy = y2 - y1;
      const length = Math.sqrt(dx * dx + dy * dy) || 1;
      // Vector unitario en la dirección de la arista
      const ux = dx / length;
      const uy = dy / length;
      // Vector normal a la arista: (-uy, ux)
      const nx = -uy;
      const ny = ux;
     ...
     * Calcula la posición para los labels early/late según el tipo.
     * Los parámetros distAlong y distNormal se definen según el label:
     *  - "earlyStart": se posiciona desde el nodo de origen.
     *  - "earlyFinish": se posiciona desde el nodo de destino.
     *  - "lateStart": se posiciona desde el nodo de origen.
     *  - "lateFinish": se posiciona desde el nodo de destino.
     */
